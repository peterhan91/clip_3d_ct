...
# Create MP4 visualizations from preprocessed CT volumes
# This script creates video animations and grid images for easy visualization

set -e  # Exit on any error

echo "=== CT-RATE Demo Visualization ==="
echo "Creating MP4 videos and grid images from HDF5 file..."
echo
...
H5_FILE="${DEMO_DIR}/demo_ct_volumes.h5"
...
if [ ! -f "${H5_FILE}" ]; then
    echo "Error: HDF5 file not found: ${H5_FILE}"
    echo "Please run the preprocessing script first:"
    echo "  ./run_demo_preprocessing.sh"
    exit 1
fi
...
# Run visualization
echo "Starting visualization creation..."
cd "${DEMO_DIR}"
...
if [ -d "${OUTPUT_DIR}" ]; then
    ...
    echo "Visualization files are ready for viewing!"
